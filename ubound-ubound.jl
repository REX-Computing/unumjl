...
function describe{ESS,FSS}(b::Ubound{ESS,FSS}, s=" ")
  ...
  hightext = is_mmr(b.highbound) ? "mmr{$ESS, $FSS}" : string(calculate(highval))
  ...
  lowtext = is_mmr(b.lowbound) ? "-mmr{$ESS, $FSS}" : string(calculate(lowval))
  lowtext = is_pos_sss(b.lowbound) ? "sss{$ESS, $FSS}" : lowtext
  lowtext = is_neg_sss(b.lowbound) ? "-sss{$ESS, $FSS}" : lowtext
  string("$(bits(b.lowbound, s)) -> $(bits(b.highbound, s)) (aka $(calculate(b.lowbound)) -> ", hightext ," )")
end
import Base.bits
