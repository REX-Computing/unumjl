...
function promote_ess{ESS,FSS}(bf, x::Unum{ESS,FSS})
  ...
  is_pos_sss(x) && return [expwalk(bf, ESS+1, FSS, max_exponent(ESS), max_exponent(ESS+1), z16),            subnormalwalk(bf, ESS+1, FSS, z16)]
  is_neg_sss(x) && return [expwalk(bf, ESS+1, FSS, max_exponent(ESS), max_exponent(ESS+1), UNUM_SIGN_MASK), subnormalwalk(bf, ESS+1, FSS, UNUM_SIGN_MASK)]
