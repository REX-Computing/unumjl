...
is_nan(x::Unum{ESS,FSS}) = is_ulp(x) && __is_nan_or_inf(x)
Base.isnan(x::Unum{ESS,FSS}) = is_ulp(x) && __is_nan_or_inf(x)
export is_nan
...
is_sss(x::Unum) = (x.exponent == z64) && is_ulp(x) && is_all_zero(x.fraction)
