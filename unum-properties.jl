...
issubnormal{ESS,FSS}(x::Unum{ESS,FSS}) = (x.exponent == 0) && !(__breakaway_checkzeros(x.fraction))
...
is_mmr{ESS,FSS}(x::Unum{ESS,FSS}) = (x.flags & UNUM_UBIT_MASK != 0) && (x.exponent == mask(1 << ESS) && (x.fraction == fillbits(-(1 << FSS - 1), length(x.fraction)))
